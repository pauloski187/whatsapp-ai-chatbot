...
class MemoryManager:
    """Store per-user message history in-memory with a 10-message cap."""
    ...
    def add_message(self, user_id: str, role: str, content: str) -> None:
        """Append one message and enforce the 10-message limit."""
    ...
    def _trim_history(self, user_id: str) -> None:
        """Keep only the latest 10 messages for a user."""
        history = self._memories.get(user_id, [])
        if len(history) > 10:
            self._memories[user_id] = history[-10:]
